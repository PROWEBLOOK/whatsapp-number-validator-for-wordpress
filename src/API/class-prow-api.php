...
class PROW_API
{
	 ...
	 * @var string
	 */
	protected $endpoint = 'https://lookup.proweblook.com/api/v1/wavalidation';
	...
	public function request($country_code = '')
	{
		...
		$response = wp_cache_get($this->get_phone(), 'prow');
		if ($response) {
		...
		if (!is_wp_error($result)) {
			...
			//wp_cache_set($this->get_phone(), $response, 'prow', 86400); // cache 24h
