...
class PROW_WPForms {
	 ...
	 * Validate form data.
	 *
    * @param  array  $fields    Sanitized entry fields. values/prowerties.
		...
		foreach ( $fields as $field_id => $field ) {
			if ( !empty( $field['value'] ) && isset( $field['type'] ) && 'phone' == $field['type'] ) {
				...
				if ( !$this->validator->get_is_valid() ) {
        				wpforms()->process->errors[ $form_id ][ $field_id ] = esc_html__( 'Please enter a valid WA number.', 'Proweblook-wa-validator' );
 				}
         }
      }  
      return $fields;
