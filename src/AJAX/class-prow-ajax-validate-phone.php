...
/**
 * Class PROW_Ajax_Validate_Phone
 */
class PROW_Ajax_Validate_Phone {
	 ...
	 * @var string
	 */
	protected $action = 'wappy-verify-phone';
	...
	public function callback() {
		...
		if ( empty( $_POST['prow-nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['prow-nonce'] ) ), 'validate-phone' ) ) { // Input var okay.
			wp_send_json( $return );
		}
		...
		//if $this->validator->get_phone_status() is empty return invalid
		if ( empty( $this->validator->get_phone_status() ) ) {
            $return['is_valid'] = false;
            $return['status']   = 'INVALID';
            $return['error']    = false;
        } else {
            $return['is_valid'] = $is_valid;
            $return['status']   = $this->validator->get_phone_status();
            $return['error']    = false;
        }
